...
/data/zoe_fastqs/multiome/ATAC/"$sample"/*R2* \
> R2_fusions.fastq

# Convert barcode fastq file to fasta, then remove fastq files
echo "Converting fastq files"
seqkit fq2fa R2_fusions.fastq -o R2_fusions.fasta
rm R2_fusions.fastq

# Trim to only have cell barcode in fasta file (reverse compliment of last 16 bp for ATAC)
fastx_trimmer -f 9 -l 24 -i R2_fusions.fasta -o R2_fusions_cellBarcode.fasta

# Take reverse complement
seqkit seq R2_fusions_cellBarcode.fasta -r -p -t "dna" -o R2_fusions_cellBarcode_revComp.fasta

# Trim to only have read name in fasta header
cut -d ' ' -f1 R2_fusions_cellBarcode_revComp.fasta | sponge R2_fusions_cellBarcode_revComp.fasta

# Convert fasta to table
seqkit fx2tab R2_fusions_cellBarcode_revComp.fasta > R2_fusions_cellBarcode.tsv
